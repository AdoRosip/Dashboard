...
- Core product:  
  \(\lambda \leftarrow \texttt{LEAGUE\_AVG\_XG} \times \text{attackStrength} \times \text{defenseWeakness}\).
...
**Headline metrics (proper 1X2):**

- Market **`1x2_multinomial`**: **multinomial Brier**  
  \(\sum_k (p_k - y_k)^2\) for \(k \in \{\text{home},\text{draw},\text{away}\}\), and **log loss** \(-\log p_{\text{actual}}\).
- Per-outcome rows **`1x2_home`**, **`1x2_draw`**, **`1x2_away`** remain for **bucket diagnostics** (binary scoring); headline Brier averages **exclude** those three in favor of **`1x2_multinomial`**, **`over25`**, **`btts_yes`** (see `calibration/metrics.ts` and `meanHeadlineBrier`).
